...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  if [[ "${SKIP_SOURCE}" != "no" ]]; then
    ...
    while [[ -n "$( git log -1 | grep "MACIDE HELPER" )" ]]; do
      git reset -q --hard HEAD~
    done
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-macide
    else
      echo "{}" > ~/.gyp/include.gypi.pre-macide
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-macide ~/.gyp/include.gypi
  fi

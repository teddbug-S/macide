...
Being a vscode based editor, Macide gets additional features by installing Visual Studio Code extensions.
...
As noted above, the [Open VSX Registry](https://open-vsx.org/) is the pre-set extension gallery in Macide. Using the extension view in Macide will therefore by default use it.
...
Or by creating a custom `product.json` at the following location (replace `Macide` by `Macide - Insiders` if you use that):
...
* [Open VSX](https://github.com/eclipse/openvsx) eclipse open-source project
  While the public instance which is run by the Eclipse Foundation is the pre-set endpoint in Macide, you can host your own instance.
